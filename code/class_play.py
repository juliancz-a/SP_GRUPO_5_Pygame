    ...
    def render(self):
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                pos = set_cards_interaction(event, card_list, letras_seleccionadas, pos)
...
def draw_cards(surface:pygame.Surface, card_list:list[Box], letras):
    ...
    for i in range (len(card_list)):
        ...
        card_list[i].assign_letter(letras[i], pos)
        pos += 1
...
def set_cards_interaction(event, card_list:list[Box], selected_letters:list, pos):
    occurrences_list = []
    back = 0
    for card in card_list:
        occurrences_list.append(card.letter)
    #hacer list a pos? bug cuando quedan espacios
    for card in card_list:
        ...
        if action:
            ...
            if selected_letters.count(card.letter) < occurrences and not card.append:
                ...
                print(card_list[card.pos].original_rectangulo.x)
