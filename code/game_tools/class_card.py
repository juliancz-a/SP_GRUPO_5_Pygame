...
class Card:
    ...
    def __init__(self, dimensiones:tuple, coords:tuple, image_path:str,
                 press_sound:str, card_pos:int, card_letter:str = None) -> None:
        """Creación de objeto tipo Card (carta) para su interacción, 
        movimiento y renderización. Comparte las propiedades de un objeto del tipo Image
        ...
        Args:
            ...
            card_pos (int): Posicion del objeto con respecto a los demás objetos 
            del mismo tipo
        ...
        self.card_image = Image(image_path, coords, dimensiones, 
                                press_sound= press_sound)
        self.card_box = self.card_image.image_box

    def draw_card(self, surface: pygame.Surface, letter_color: str | tuple, 
                  font: str, font_size: int, transparency: int = 255):
        """Dibujar en pantalla una carta.

        Args:
            surface (pygame.Surface): Superficie sobre la cual realizar el dibujo.
            letter_color (str, tuple): Color de la letra perteneciente a la carta
        ...
        self.card_box.draw_text(surface, self.letter, letter_color, font, font_size,
                                outline="border", outline_thickness=2, center=True)